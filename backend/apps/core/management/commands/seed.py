...
class Command(BaseCommand):
    ...
    def _plans(self):
        ...
        plans = [
            dict(name="free",       display_name="Free",       price=0,   max_cameras=1,   max_users=2,   max_rules=3,   trial_days=30, sort_order=0, features={},                                                              is_active=True),
            dict(name="starter",    display_name="Starter",    price=99,  max_cameras=5,   max_users=5,   max_rules=10,  trial_days=30, sort_order=1, features={},                                                              is_active=True),
            dict(name="pro",        display_name="Pro",        price=249, max_cameras=15,  max_users=15,  max_rules=50,  trial_days=30, sort_order=2, features={"ai_vision": True},                                             is_active=True),
            dict(name="enterprise", display_name="Enterprise", price=699, max_cameras=100, max_users=100, max_rules=999, trial_days=30, sort_order=3, features={"ai_vision": True, "api_access": True, "white_label": True},    is_active=True),
        ]
        for p in plans:
    ...
    def _rules(self, tenant):
        ...
        # Base rules for all plans
        rules = [
            ("Zona restrita",           "restricted_zone",    "critical", ["telegram","email"], 60,  {"zones":[]}),
            ("Aglomeração",             "crowding",           "high",     ["telegram"],          90,  {"crowd_count":5}),
            ("Permanência prolongada",  "loitering",          "high",     ["email"],             120, {"loitering_seconds":300}),
            ("Movimento noturno",       "night_movement",     "critical", ["telegram","email"],  30,  {"schedule_start":22,"schedule_end":6}),
            ("Detecção de movimento",   "motion",             "low",      ["email"],             300, {}),
        ]

        # Starter+
        if tenant.plan and tenant.plan.max_rules >= 10:
            rules += [
                ("Queda / pessoa caída",        "fall_detection",    "critical", ["telegram","email"], 30,  {}),
                ("Ausência de EPI",             "missing_ppe",       "high",     ["telegram"],          60,  {}),
                ("Contador de pessoas",         "people_counter",    "low",      ["email"],             60,  {"max_people":20}),
                ("Fila longa no caixa",         "queue_detection",   "medium",   ["email"],             120, {"queue_limit":5}),
                ("Objeto abandonado",           "abandoned_object",  "high",     ["telegram","email"],  90,  {}),
            ]

        # Pro+
        if tenant.plan and tenant.plan.max_rules >= 50:
            rules += [
                ("Postura suspeita de furto",   "shoplifting_posture","critical", ["telegram","email"], 45,  {}),
                ("Mochila grande em zona",      "large_bag",         "medium",   ["telegram"],          120, {}),
                ("Pessoa correndo",             "running",           "medium",   ["telegram"],          60,  {}),
                ("Veículo em área de pedestre", "vehicle_pedestrian","high",     ["telegram","email"],  60,  {}),
                ("Invasão de perímetro",        "perimeter_breach",  "critical", ["telegram","email"],  30,  {}),
                ("Passagem não autorizada",     "tailgating",        "high",     ["telegram"],          60,  {"max_allowed":1}),
                ("Criança desacompanhada",      "lone_child",        "high",     ["telegram","email"],  60,  {}),
                ("Risco em piscina",            "pool_risk",         "critical", ["telegram","email"],  20,  {}),
                ("Permanência em banheiro",     "bathroom_loiter",   "medium",   ["telegram"],          300, {"minutes":10}),
                ("Pessoa imóvel",               "motionless_person", "critical", ["telegram","email"],  60,  {}),
                ("Veículo em zona proibida",    "vehicle_zone",      "high",     ["telegram","email"],  60,  {}),
                ("Sem capacete",                "no_hardhat",        "high",     ["telegram"],          60,  {}),
                ("Animal detectado",            "animal_detection",  "medium",   ["email"],             120, {}),
                ("Veículo sentido errado",      "wrong_way",         "critical", ["telegram","email"],  30,  {}),
                ("Estacionamento proibido",     "parking_violation", "medium",   ["email"],             300, {}),
            ]
    ...
    def _alerts(self, tenant):
        ...
        descs = {
            "restricted_zone":    ["Pessoa em área restrita","Acesso não autorizado","Funcionário em área proibida"],
            "crowding":           ["6 pessoas detectadas","Aglomeração: 8 pessoas","Limite de lotação atingido"],
            "loitering":          ["Pessoa parada 5min","Indivíduo suspeito 8min","Permanência prolongada"],
            "night_movement":     ["Movimento às 02:34","Pessoa fora do horário","Atividade noturna suspeita"],
            "motion":             ["Movimento detectado","Objeto em movimento","Sensor ativado"],
            "fall_detection":     ["Possível queda","Pessoa no chão","Queda detectada"],
            "missing_ppe":        ["Sem capacete","Sem colete","EPI incompleto"],
            "ai_vision":          ["IA: comportamento suspeito","IA: possível conflito","IA: situação de risco"],
            "people_counter":     ["15 pessoas no local","Fluxo elevado detectado","Ocupação acima do limite"],
            "queue_detection":    ["Fila com 7 pessoas","Espera longa no caixa","Fila ultrapassou limite"],
            "abandoned_object":   ["Mochila sem dono detectada","Objeto abandonado há 5min","Bagagem suspeita"],
            "shoplifting_posture":["Pessoa agachada suspeita","Postura de furto detectada","Comportamento suspeito"],
            "large_bag":          ["Mochila grande em zona monitorada","Bolsa suspeita detectada"],
            "running":            ["Pessoa correndo no corredor","Corrida detectada em área interna"],
            "vehicle_pedestrian": ["Moto em calçada","Carro em corredor de pedestre"],
            "perimeter_breach":   ["Invasão de perímetro às 23h","Pessoa em área externa"],
            "tailgating":         ["2 pessoas passando juntas","Acesso não autorizado detectado"],
            "lone_child":         ["Criança sozinha detectada","Menor desacompanhado"],
            "pool_risk":          ["Pessoa na borda da piscina","Criança na beira sem adulto"],
            "bathroom_loiter":    ["Pessoa há 12min no banheiro","Permanência suspeita detectada"],
            "motionless_person":  ["Pessoa sem movimento há 3min","Possível desmaio detectado"],
            "vehicle_zone":       ["Empilhadeira em zona de pedestres","Veículo em área proibida"],
            "no_hardhat":         ["Trabalhador sem capacete","EPI — sem capacete detectado"],
            "animal_detection":   ["Cachorro em área restrita","Animal detectado na câmera"],
            "wrong_way":          ["Veículo em mão contrária","Carro em sentido proibido"],
            "parking_violation":  ["Carro em vaga proibida há 15min","Estacionamento irregular"],
            "overcrowded_vehicle":["6 pessoas ao redor do veículo","Excesso de ocupantes detectado"],
        }
