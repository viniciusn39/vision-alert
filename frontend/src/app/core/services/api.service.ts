...
export class ApiService {
  ...
  /**
   * Pede um stream-token curto (60s) para abrir o MJPEG ao vivo.
   * O token é amarrado a user+camera e é descartável — não é o JWT de sessão.
   */
  requestStreamToken(cameraId: number): Observable<{ token: string; expires_in: number }> {
    return this.http.post<{ token: string; expires_in: number }>(
      `${this.b}/cameras/${cameraId}/stream-token/`, {}
    );
  }

  /**
   * Constrói a URL MJPEG usando o stream-token (query ?st=...).
   * O front deve chamar requestStreamToken antes e passar o resultado aqui.
   */
  getCameraStreamUrl(cameraId: number, streamToken: string): string {
    return `${this.b}/cameras/${cameraId}/stream/?st=${encodeURIComponent(streamToken)}`;
  }
  ...
  rotateDeviceKey(id: string) { return this.http.post<{ api_key: string }>(`${this.b}/fleet/admin/devices/${id}/rotate-key/`, {}); }
